...
var rotInput = Enumerable.Range(0, colCount).Select(x => Enumerable.Range(0, rowCount).Select(y => input[y][x]).ToArray()).ToArray();
var p1GammaStr = "";
...
Console.WriteLine($"Part 2: {Day3.LifeSupport.GetRating(input, true) * Day3.LifeSupport.GetRating(input, false)}");
