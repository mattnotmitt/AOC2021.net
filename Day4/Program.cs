...
namespace Day4 // Note: actual namespace depends on the project name.
{
    ...
    {
        private List<int[][]> boards = new();
        private int[] callouts = Array.Empty<int>();
        private List<int> winState = new();
        private List<int> winOrder = new();
        ...
        {
            var rotBoard = Enumerable.Range(0, 5).Select(x => Enumerable.Range(0, 5).Select(y => board[y][x]).ToArray()).ToArray();
